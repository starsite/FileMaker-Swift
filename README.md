...
Data API v19 or later. In prior versions, we had to attempt (and retry) calls that failed, or set and track an expiry `Date()`. Neither of those options were great. Now we can (very) quickly validate a session token. 🎉
