...
I updated SwiftFM this weekend to include the new `validateSession()` method. We no longer need to track expiry values with `isActiveToken()`. 🎉 Because Claris is using a slightly different URL path for validations, I also went ahead and refactored the `URLSession` calls to use `host`, `db`, and `auth` environment values. That's a better way to do it anyway.
