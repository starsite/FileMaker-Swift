...
💡 I've included an example of a Swift "trailing closure" in the code example. Trailing closures are _everywhere_ in Swift and SwiftUI, so you should get used to seeing them, and writing them. They're pretty great. Functions are still defined with a `completion:` parameter. You opt for a trailing closure when you make the call by double-clicking Xcode's `completion:` placeholder. Or you can tab to it and hit `Return`.
