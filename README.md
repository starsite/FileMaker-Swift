...
I updated SwiftFM this weekend to include the new `validateSession()` method. We no longer need to track expiry values with `isActiveToken()`. 🎉 However, since Claris uses a slightly different URL path for validations, I went ahead and refactored all the `URLSession` calls to use `host`, `db`, and `auth` environment values. That's a better way to do it anyway.
